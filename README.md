...
| **Week 3**    | ทำส่วน profile user && update profile         | สร้างไฟล์JSONที่disignไว้และใส่ข้อมูลuserลงไป && design path routing && ทำระบบ login(get&post), signin(get&post), studentHome(get) และ fetchAPI && สร้างระบบเก็บข้อมูล login user ลง cookie และผูกกับ pinia เพื่อเรียกใช้ && navBar(getข้อมูลชื่อuserที่loginจากpinia)            | ทำฟีเจอร์ในการสร้าง/แก้ไข/ลบ ข้อสอบ(ยังไม่สมบูรณ์)         | ทำฟีดเจอร์เช็คคำตอบ กับคะแนน (ยังไม่สมบูรณ์)           |
| **Week 4**    | -          | -          | -          | -          |
| **Week 5**    | -          | -          | -          | -          |
