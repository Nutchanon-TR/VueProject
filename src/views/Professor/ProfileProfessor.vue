            ...
            <div v-for="(exam, index) in ownedExams" :key="index"
              ...
              <div class="px-5 py-4 relative">
                <RouterLink :to="{ name: 'EditExamPage', params: { quizId: exam.id } }">
                  <h2 class="text-2xl sm:text-3xl font-bold text-black truncate">{{ exam.category }}</h2>
                  <p class="font-semibold pt-2 pb-8 text-lg sm:text-xl truncate">
                    {{ exam.name }}
                  </p>
                </RouterLink>

                <button @click="deleteExam(exam.id)"
                  class="absolute bottom-2 right-2 p-2 rounded-full text-white text-2xl sm:text-3xl hover:bg-red-500 transition">
                  🗑️
                </button>
              </div>
              ...
              <div class="flex justify-between items-center bg-white p-3 text-black border-t w-full">
                <div class="flex items-center">
                  <span class="text-red-500 text-lg sm:text-xl">❤️</span>
                  <span class="ml-2 text-lg sm:text-2xl font-semibold">{{ exam.likes }}</span>
                </div>
              </div>
            </div>
          </div>
